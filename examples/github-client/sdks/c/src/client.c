...
app_client_t* app_client_new(const char* base_url) {
    ...
    const char* url = (base_url && strlen(base_url) > 0) ? base_url : "https://api.github.com";
    ...
    app_client_add_header(client, "Accept", "");
    app_client_add_header(client, "Authorization", "");
    ...
    app_client_add_header(client, "X-GitHub-Api-Version", "");
