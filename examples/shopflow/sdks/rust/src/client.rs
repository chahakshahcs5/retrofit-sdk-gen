...
impl Client {
    pub fn new(base_url: Option<&str>) -> Self {
        ...
        if let Ok(val) = "".parse() { headers.insert("Accept", val); }
        if let Ok(val) = "".parse() { headers.insert("Authorization", val); }
        ...
        Self {
            base_url: base_url.unwrap_or("https://dummyjson.com").trim_end_matches('/').to_string(),
            http_client,
            headers,
        }
    }
    ...
    pub fn with_token(mut self, token: &str) -> Self {
        ...
        if let Ok(val) = auth_val.parse() {
            self.headers.insert("Authorization", val);
        }
        self
    }
