...
export function generateCSharpClient(
  baseUrl: string,
  securityResult?: SecurityScanResult
): string {
  const cleanUrl = (baseUrl || "https://api.example.com").replace(/\/+$/, "");

  const headerLines: string[] = ['            DefaultHeaders["Content-Type"] = "application/json";'];
    ...
    for (const h of securityResult.detectedHeaderNames) {
      if (!skip.has(h.toLowerCase())) {
        if (h.toLowerCase() === "user-agent") {
          headerLines.push(`            DefaultHeaders["${h}"] = "Mozilla/5.0 (Android; Mobile)";`);
        } else if (h.toLowerCase().includes("priority")) {
          headerLines.push(`            DefaultHeaders["${h}"] = "normal";`);
        } else {
          headerLines.push(`            DefaultHeaders["${h}"] = "";`);
        }
      }
    }
  }
...
namespace App.Sdk
{
    ...
    public class HttpClientWrapper
    {
        ...
        {
            BaseUrl = (baseUrl ?? "${cleanUrl}").TrimEnd('/');
